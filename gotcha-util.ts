...
export default class GotchaUtil{
    ...
    public static init() {
        ...
        let baseUrl = config.params.baseUrl;
        let vid = config.params.vid;
        let isVe = config.params.isVe;
        let system = isVe ? 've' : 'primo';

        fs.writeFile('gotcha.json', "{}", 'utf8', ()=>{console.log("file written to root as {}")});
                ...
                const jwtResponse:ResponsePromise = <ResponsePromise>http.get(jwtPath[system]).then(
                    (jwtResponse)=> {
                        ...
                        function callback(error, gotchaResponse, body) {
                            ...
                            fs.writeFile('gotcha.json', gotcha, 'utf8', ()=>{console.log("file written to root with data: " + gotcha)});
                            fs.writeFile('tmp/gotcha.json', gotcha, 'utf8', ()=>{console.log("file written to tmp/gotcha.json")});
